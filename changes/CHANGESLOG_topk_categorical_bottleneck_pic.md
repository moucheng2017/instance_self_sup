# Soft Categorical Bottleneck PIC
...
- Added `topk_categorical_bottleneck_pic_net`, a PIC-style instance classifier
  with a deterministic soft categorical bottleneck between backbone features
  and instance-ID logits.
- Added focused tests in `tests/test_topk_categorical_bottleneck_pic.py` for
  soft assignment probabilities, column normalization, and model forward
  outputs.
...
capacity-controlled soft categorical bottleneck, allowing a sweep over latent
capacity `C` to test whether high-capacity instance classification drives the
kNN-minus-linear-probe gap.
